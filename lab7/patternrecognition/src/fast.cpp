...
//        double slopeToOrigo;
//        unordered_map<double, vector<Point>> sortedPoints;
//        Point origo = points[i];

//        for(int j = i + 1; j < points.size(); j++){
//            slopeToOrigo = points[j].slopeTo(origo);
//            sortedPoints[slopeToOrigo].insert(sortedPoints[slopeToOrigo].begin(), points[j]);

//            if(sortedPoints[slopeToOrigo].size() > 2){
//                render_line(scene, origo, sortedPoints[slopeToOrigo].at(0));
//                a.processEvents(); // show rendered line
//            }
//        }
////        for(auto it = sortedPoints.begin(); it != sortedPoints.end(); it++){
////            counter = sortedPoints[it->first].size();
////            if(counter > 2){
